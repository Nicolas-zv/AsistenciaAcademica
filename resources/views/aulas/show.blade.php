{{-- resources/views/aulas/show.blade.php (ADAPTADO) --}}
            ...
            <div class="p-4 sm:p-8">
                ...
                <div class="bg-gray-800 p-6 rounded-lg shadow-xl border border-gray-700 max-w-3xl mx-auto">
                    ...
                    <div class="space-y-4">
                        ...
                        {{-- Número --}}
                        <div class="flex items-start pb-2 border-b border-gray-700">
                        ...
                        {{-- Tipo y Capacidad (agrupados) --}}
                        <div class="grid grid-cols-2 gap-4 pb-2 border-b border-gray-700">
                            <div>
                                <p class="text-sm font-medium text-gray-400">Tipo de Aula:</p>
                                <p class="text-base text-gray-300">{{ $aula->tipo ?? 'No especificado' }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-400">Capacidad:</p>
                                <p class="text-base text-gray-300">{{ $aula->capacidad ?? 'N/A' }} personas</p>
                            </div>
                        </div>
                    ...
                    <div class="mt-8 pt-4 border-t border-gray-700 text-xs text-gray-500 space-y-1">
                        <p>Registro Creado: **{{ $aula->created_at->format('d/M/Y H:i') }}** ({{ $aula->created_at->diffForHumans() }})</p>
                        <p>Última Actualización: **{{ $aula->updated_at->format('d/M/Y H:i') }}** ({{ $aula->updated_at->diffForHumans() }})</p>
                    </div>
