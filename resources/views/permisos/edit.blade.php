        ...
        <form method="POST" action="{{ route('permisos.update', $permiso) }}" 
            ...
            <div class="mb-6">
                ...
                <textarea name="descripcion" id="descripcion" rows="3"
                          class="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:border-indigo-500 focus:ring-indigo-500 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $permiso->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                @enderror
            </div>
