        ...
        <div class="bg-gray-800 p-6 rounded-lg shadow-xl border border-gray-700 max-w-3xl mx-auto">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-start">
                    ...
                    <p class="w-2/3 text-base text-white font-bold">{{ $modulo->nombre }}</p>
                </div>
                
                {{-- Código --}}
                <div class="flex items-start">
                    <p class="w-1/3 text-sm font-medium text-gray-400">Código:</p>
                    <p class="w-2/3 text-base text-indigo-400 font-bold">{{ $modulo->codigo ?? 'N/A' }}</p>
                </div>
            ...
            <div class="mt-8 pt-4 border-t border-gray-700 text-xs text-gray-500">
                <p>Registro Creado: {{ $modulo->created_at->diffForHumans() }} ({{ $modulo->created_at->format('d/m/Y H:i') }})</p>
                <p>Última Actualización: {{ $modulo->updated_at->diffForHumans() }} ({{ $modulo->updated_at->format('d/m/Y H:i') }})</p>
            </div>
