{{-- resources/views/grupo_materia/edit.blade.php --}}
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="p-4 sm:p-8">
                ...
                <h1 class="text-3xl font-extrabold text-white dark:text-white mb-6 border-b border-gray-700 pb-3">
                    Editar Asignación:
                    **{{ $grupoMateria->materia->sigla ?? 'N/A' }}-{{ $grupoMateria->grupo->nombre ?? 'N/A' }}**
                </h1>

                <div class="bg-gray-800 p-6 rounded-lg shadow-xl border border-gray-700 max-w-4xl mx-auto">

                    {{-- ✅ CORRECCIÓN CLAVE: Usar la clave explícita 'grupo_materium' para resolver el error de parámetro faltante --}}
                    <form method="POST" action="{{ route('grupo_materia.update', ['grupo_materia' => $grupoMateria]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-gray-700 pb-6">
                                <h2 class="col-span-full text-lg font-semibold text-indigo-400 mb-2">Identificación
                                    Académica (*)</h2>
                                ...
                                {{-- Materia --}}
                                <div>
                                    <label for="materia_id" class="block text-sm font-medium text-gray-300 mb-1">Materia
                                        (*)</label>
                                    <select name="materia_id" id="materia_id" required
                                        ...
                                        @foreach ($materias as $materia)
                                            <option value="{{ $materia->id }}"
                                                {{ old('materia_id', $grupoMateria->materia_id) == $materia->id ? 'selected' : '' }}>
                                ...
                                {{-- Grupo --}}
                                <div>
                                    <label for="grupo_id" class="block text-sm font-medium text-gray-300 mb-1">Grupo
                                        (*)</label>
                                    <select name="grupo_id" id="grupo_id" required
                                        ...
                                        @foreach ($grupos as $grupo)
                                            <option value="{{ $grupo->id }}"
                                                {{ old('grupo_id', $grupoMateria->grupo_id) == $grupo->id ? 'selected' : '' }}>
                                                {{ $grupo->nombre }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                {{-- Gestión --}}
                                <div>
                                    <label for="gestion_id" class="block text-sm font-medium text-gray-300 mb-1">Gestión
                                        (*)</label>
                                    <select name="gestion_id" id="gestion_id" required
                                        ...
                                        @foreach ($gestiones as $gestion)
                                            <option value="{{ $gestion->id }}"
                                                {{ old('gestion_id', $grupoMateria->gestion_id) == $gestion->id ? 'selected' : '' }}>
                            ...
                            {{-- 🧩 Bloque 1.5: Docente Asignado --}}
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-gray-700 pb-6">
                                <h2 class="col-span-full text-lg font-semibold text-green-400 mb-2">
                                    Asignación de Docente
                                </h2>

                                <div class="md:col-span-2">
                                    <label for="docente_id" class="block text-sm font-medium text-gray-300 mb-1">
                                        Docente Asignado
                                    </label>

                                    <select name="docente_id" id="docente_id"
                                        class="w-full bg-gray-700 border border-gray-600 text-white rounded-md shadow-sm p-3
                   focus:ring-green-500 focus:border-green-500
                   @error('docente_id') border-red-500 @enderror">

                                        {{-- Opción vacía --}}
                                        <option value=""
                                            {{ old('docente_id', $grupoMateria->docente_id) == null ? 'selected' : '' }}>
                                            -- Sin Asignar Docente --
                                        </option>
                                        ...
                                        @foreach ($docentes as $docente)
                                            <option value="{{ $docente->id }}"
                                                {{ old('docente_id', $grupoMateria->docente_id) == $docente->id ? 'selected' : '' }}>
                                                {{ $docente->user->nombre ?? 'Docente sin nombre' }}
                                                ({{ $docente->user->correo ?? 'sin correo' }})
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 border-b border-gray-700 pb-6">
                                <h2 class="col-span-full text-lg font-semibold text-gray-300 mb-2">Detalles y Ubicación
                                </h2>

                                {{-- Turno --}}
                                <div>
                                    <label for="turno"
                                        class="block text-sm font-medium text-gray-300 mb-1">Turno</label>
                                    <select name="turno" id="turno"
                                        ...
                                        <option value=""
                                            {{ old('turno', $grupoMateria->turno) == '' ? 'selected' : '' }}>--
                                            Seleccione --</option>
                                        <option value="Mañana"
                                            {{ old('turno', $grupoMateria->turno) == 'Mañana' ? 'selected' : '' }}>
                                            Mañana</option>
                                        <option value="Tarde"
                                            {{ old('turno', $grupoMateria->turno) == 'Tarde' ? 'selected' : '' }}>Tarde
                                        </option>
                                        <option value="Noche"
                                            {{ old('turno', $grupoMateria->turno) == 'Noche' ? 'selected' : '' }}>Noche
                                        </option>
                                    </select>
                                    @error('turno')
                                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                                ...
                                {{-- Cupo --}}
                                <div>
                                    <label for="cupo" class="block text-sm font-medium text-gray-300 mb-1">Cupo
                                        Máximo</label>
                                ...
                                {{-- Módulo Asignado --}}
                                <div>
                                    <label for="modulo_id" class="block text-sm font-medium text-gray-300 mb-1">Módulo
                                        Asignado</label>
                                    <select name="modulo_id" id="modulo_id"
                                        ...
                                        @foreach ($modulos as $modulo)
                                            <option value="{{ $modulo->id }}"
                                                {{ old('modulo_id', $grupoMateria->modulo_id) == $modulo->id ? 'selected' : '' }}>
                                ...
                                {{-- Aula Asignada --}}
                                <div>
                                    <label for="aula_id" class="block text-sm font-medium text-gray-300 mb-1">Aula
                                        Asignada</label>
                                    <select name="aula_id" id="aula_id"
                                        ...
                                        @foreach ($aulas as $aula)
                                            <option value="{{ $aula->id }}"
                                                {{ old('aula_id', $grupoMateria->aula_id) == $aula->id ? 'selected' : '' }}>
                                                N° {{ $aula->numero }} ({{ $aula->modulo->codigo ?? 'M.N/A' }}) -
                                                {{ $aula->tipo }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="md:col-span-4">
                                    <label for="estado" class="block text-sm font-medium text-gray-300 mb-1">Estado
                                        (*)</label>
                                    <select name="estado" id="estado" required
                                        ...
                                        <option value="activo"
                                            {{ old('estado', $grupoMateria->estado) == 'activo' ? 'selected' : '' }}>
                                            Activo</option>
                                        <option value="inactivo"
                                            {{ old('estado', $grupoMateria->estado) == 'inactivo' ? 'selected' : '' }}>
                                            Inactivo (En espera)</option>
                                        <option value="cerrado"
                                            {{ old('estado', $grupoMateria->estado) == 'cerrado' ? 'selected' : '' }}>
                                            Cerrado (Finalizado)</option>
                                    </select>
                                    @error('estado')
                                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                                    @enderror
                        ...
                        <div class="mt-8 flex justify-end space-x-4 border-t border-gray-700 pt-6">
                            {{-- ✅ CORRECCIÓN B: Aplicar la sintaxis explícita también al enlace Cancelar por consistencia --}}
                            <a href="{{ route('grupo_materia.show', ['grupo_materia' => $grupoMateria]) }}"
